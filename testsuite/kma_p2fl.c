...
#define FALSE 0
#define TRUE  1
#define MAXSPACE (PAGESIZE - sizeof(kma_page_t*) - sizeof(kflHeader_t) - sizeof(bufHeader_t))
#define MAXSET 9
#define BUFSIZE0 1<<5
#define BUFSIZE1 1<<6
#define BUFSIZE2 1<<7
...
kma_free(void* ptr, kma_size_t size)
{
	...
	if(size > MAXSPACE / 2)
	{
		kma_page_t* page = *((kma_page_t**)((void*)ptr - sizeof(kma_page_t*)));
		free_page(page);
		return;
	}
...
cleanupKFL()
{
	if(kflPtr->spaceUsed == 0)
	{
		...
		kma_page_t* tempPagePtr = NULL;
		while(tempKflPtr != NULL)
		{
			tempPagePtr = *((kma_page_t**)((void*)tempKflPtr - sizeof(kma_page_t*)));
...
int initKFL(kma_size_t size)
{
	...
	*((kma_page_t**)page->ptr) = page;
	...
	if((size + sizeof(kma_page_t*) + sizeof(kflHeader_t)) > page->size)
	{
		free_page(page);
		return -1;
	}
	...
	curKflPtr = page->ptr + sizeof(kma_page_t*);

	curKflPtr->freespaceSize = page->size - sizeof(kma_page_t*) - sizeof(kflHeader_t);
	curKflPtr->freespacePtr = page->ptr + sizeof(kma_page_t*) + sizeof(kflHeader_t);
