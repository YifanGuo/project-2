...
void
get_buffer_from_large_buffer(void* pagePtr, kma_size_t reqBufSize, unsigned char delayed, kma_size_t largeBufSize, kma_size_t largeBufStartAddr);
bufferHeader_t*
find_buffer_in_free_list(void* pagePtr, void* bufPtr, unsigned char bufClass, unsigned char delayed);
void
add_buffer_to_free_list_front(void* pagePtr, unsigned char bufClass, unsigned char delayed, kma_size_t startAddr, kma_size_t bufSize);
void
add_buffer_to_free_list_back(void* pagePtr, unsigned char bufClass, unsigned char delayed, kma_size_t startAddr, kma_size_t bufSize);
void
...
kma_malloc(kma_size_t size)
{
  ...
  if (budfls == NULL) /* if size is too large for keeping a page header, return */
  {
    if (!init(size)) 
    {
      return NULL;
...
kma_free(void* ptr, kma_size_t size)
{
  ...
  if (size > PAGESIZE / 2) 
  {
    ...
    {
      if (firstPageSpaceUsed == 0 && pagesUsed == 1) 
      {
  ...
  if (((pageHeader_t*)pagePtr)->spaceUsed == 0) 
  {
    ...
    if (pagePtr != firstPagePtr) 
    {
      if (((pageHeader_t*)(budfls->firstPagePtr))->spaceUsed == 0 &&budfls->pagesUsed == 1) 
      {
...
header_alloc(void* pagePtr, kma_size_t headerSize)
{
  ...
  memset(pagePtr + sizeof(kma_page_t*), '\000', MAXBITMAPSEGS);
...
big_size_alloc(kma_size_t reqSize)
{
  ...
  *((kma_page_t**)page->ptr) = page;
  ...
  return page->ptr + sizeof(kma_page_t*) + sizeof(bitMap_t);
}
...
buddy_alloc(kma_size_t reqSize)
{
  ...
  else 
  {
    ...
    { 
      ...
      get_buffer_from_large_buffer(((bufferHeader_t*)bufPtr)->pagePtr, reqBufSize, delayed, (budfls->fl[bufClass]).size, (kma_size_t)(bufPtr - ((bufferHeader_t*)bufPtr)->pagePtr));
      if (delayed == 0) 
      {
	      update_bitmap(((bufferHeader_t*)bufPtr)->pagePtr, bufPtr, reqBufSize, USED);
      }
...
coalesce(void* pagePtr, void* bufPtr, kma_size_t bufSize)
{
  ...
  void* buddyPtr = bufStartAddr % (bufSize * 2) == bufSize ? bufPtr - bufSize : bufPtr + bufSize;
  ...
  bufferHeader_t* buddyHdrPtr = find_buffer_in_free_list(pagePtr, buddyPtr, bufClass, 1);
...
get_buf_class(kma_size_t bufSize)
{
  ...
  // replace the function of log
  int tmp = PAGESIZE / bufSize;
  int count = 0;
  while (tmp >1)
  {
    tmp /= 2;
  ...
  return (unsigned char)((int)(count) - 1);
}
...
get_buf_size(unsigned char bufClass)
{
  ...
  // replace fuction of power
  int temp = 1;
  int classcount = MAXBUFCLASS - 1 - (int)bufClass;
  int cont = 0;
  while (cont < classcount)
  {
    temp *= 2;
    cont++;
  }
  return (int) (temp * MINBUFSIZE);
}
...
void
add_buffer_to_free_list_front(void* pagePtr, unsigned char bufClass, unsigned char delayed, kma_size_t bufStartAddr, kma_size_t bufSize)
{
...
void
add_buffer_to_free_list_back(void* pagePtr, unsigned char bufClass, unsigned char delayed, kma_size_t bufStartAddr, kma_size_t bufSize)
{
...
update_bitmap(void* pagePtr, void* bufPtr, kma_size_t bufSize, bool status)
{
  ...
  bitMap_t* bitMapLoc = pagePtr + sizeof(kma_page_t*);
